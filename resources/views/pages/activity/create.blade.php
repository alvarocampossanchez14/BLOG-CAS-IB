...
<div class="max-w-4xl mx-auto px-4 py-8 bg-background rounded-lg shadow-xl my-8">
    <h1 class="text-3xl font-bold mb-6 text-center text-text">Crear Nueva Actividad</h1>
    ...
    <form action="{{ route('activities.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-6"> {{-- Increased space-y for better visual separation --}}
                ...
                <div>
                    ...
                    <textarea name="evaluation" id="evaluation" rows="4"
                              class="mt-1 block w-full bg-gray-800 border-gray-600 rounded-md shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 px-3 py-2 text-text">{{ old('evaluation') }}</textarea>
                </div>
                <div>
        ...
        <div class="flex justify-center mt-8">
            <a href="{{ route('activities.index') }}" class="px-6 py-3 bg-gray-600 text-white font-semibold rounded-md shadow-lg hover:bg-gray-700 transition duration-300 mr-4">
                Cancelar
            </a>
